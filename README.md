...
docker env:```docker run -itd --platform linux/arm64 --name wlt -v /usr/bin/qemu-aarch64-static:/usr/bin/qemu-aarch64-static -v /root/wlt-rt:/root ubnutuarm:24.04 bash```


对于无法交互的自动化脚本环境必须加入变量```export DEBIAN_FRONTEND=noninteractive```
...
apt update
apt install --no-install-recommends -y apt-fast
```
